...
export default function App(){
  ...
  async function verifyPin(){
    ...
    if(pinInput===correct){
      ...
      await refreshAdmin(); // includes keepalive ping to Supabase
      setView('admin:list');
    } else {
  ...
  return <div style={s.layout}>
    ...
    <header style={s.header}>
      ...
      {ev?.status&&ev.status!=='draft'&&view!=='admin:list'&&view!=='admin:newform'&&view!=='admin:editform'&&view!=='admin:forms'&&<button onClick={()=>openPrint(view)}
        style={{...s.btnSm,gap:5}}>
    ...
    <div style={s.main}>
      ...
      {view==='home'&&<div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
          {[{
            ...
            title:'Soy Evaluador',
            sub:'Líderes, Supervisores & Encargados de Área',
            target:'eval:type',
            ...
            title:'Soy Evaluado',
            sub:'Personal Operativo — Revisar y aceptar mi evaluación',
            ...
            <div style={{fontSize:16,fontWeight:700,color:TX,marginBottom:4}}>{x.title}</div>
            <div style={{fontSize:11,fontWeight:600,color:x.color,textTransform:'uppercase',letterSpacing:'0.04em',lineHeight:1.5}}>{x.sub}</div>
          </button>)}
        </div>
      ...
      {view==='admin:list'&&<div>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={async(e)=>{
              ...
              btn.disabled=true;
              try{
                await supabase.from('config').upsert({key:'keepalive',value:new Date().toISOString(),site:'global'});
                const d=new Date(); d.setDate(d.getDate()+7);
                setKeepaliveMsg('✓ Activo hasta ~'+d.toLocaleDateString('es-PE',{day:'2-digit',month:'short'}));
                setTimeout(()=>setKeepaliveMsg(''),6000);
              }catch(ex){setKeepaliveMsg('⚠ Error: '+ex.message);}
              btn.textContent='🔋 Mantener activo';
              btn.disabled=false;
            }} title="Registra actividad en Supabase para evitar suspensión" style={{...s.btnSm,display:'flex',alignItems:'center',gap:4}}>
              🔋 Mantener activo
            </button>
        ...
        {keepaliveMsg&&<div style={{background:keepaliveMsg.startsWith('✓')?GBKG:RBKG,border:`1px solid ${keepaliveMsg.startsWith('✓')?GBD:RBD}`,borderRadius:8,padding:'8px 16px',marginBottom:12,fontSize:12,color:keepaliveMsg.startsWith('✓')?G:R,fontWeight:500}}>{keepaliveMsg}</div>}
